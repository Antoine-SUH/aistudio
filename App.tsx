...
const App: React.FC = () => {
  ...
  // Lifted from Editor.tsx to support multi-view architecture
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setError({ title: "Load Error", message: err.message || "Unknown error" });
      setFileLoaded(false);
    }
  };

  const handleTagsUpdated = () => {
    // Refresh tags from manager when Creator modifies the file
    try {
  ...
  // Clears the file from memory but keeps the current view
  ...
  // Intercepts the view change request
  const requestViewChange = (newView: ViewMode) => {
    if (newView === currentView) return;

    // If a file is loaded, warn the user
    if (fileLoaded) {
        ...
        {!fileLoaded ? (
            ...
            <div className="flex flex-col items-center justify-center h-full p-6">
                <div className="bg-white p-10 rounded-xl shadow-xl text-center max-w-lg w-full border border-slate-200 relative">
                    ...
                    <h2 className="text-2xl font-bold text-slate-800 mb-2">
                        {currentView === 'generator' ? 'Upload Template' : 'Create Template'}
                    </h2>
                    <p className="text-slate-500 mb-8">
                        {currentView === 'generator' 
                            ? 'Upload a DOCX file to start generating forms.'
                            : 'Upload a DOCX file to start converting text into tags.'
                        }
                    </p>
            ...
            <>
                <div className="absolute top-4 right-4 z-50">
                     <button onClick={resetFileState} className="bg-white p-2 rounded-full shadow text-slate-400 hover:text-red-500 transition-colors" title="Close File">
                        <Trash2 size={20} />
                    </button>
                </div>
                  ...
                  <div className="flex items-start gap-4 mb-4">
                      ...
                      <div>
                          ...
                          <p className="text-slate-600 mt-2 text-sm leading-relaxed">
                            Smart Docx Editor n'enregistre rien, veillez à bien télécharger vos documents modifiés.
                          </p>
                      </div>
                  </div>
